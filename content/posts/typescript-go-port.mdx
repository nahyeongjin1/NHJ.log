...
그래서 Effective TypeScript를 읽어보던 중 <strong>tsc</strong>와 <strong>tsserver</strong>의 구현이 궁금해졌고 다음 레포지토리를 뜯어보기로 했다.

<LinkCard url="https://github.com/microsoft/TypeScript" title="GitHub - microsoft/TypeScript: TypeScript is a superset of JavaScript that compiles to clean JavaScript output." description="TypeScript is a superset of JavaScript that compiles to clean JavaScript output. - microsoft/TypeScript" image="https://opengraph.githubassets.com/ca31ae76312f07e53893091d8cc4179727c2f2280641ad055286023fa835d334/microsoft/TypeScript" favicon="https://github.githubassets.com/favicons/favicon.svg" />
...
- <strong>tsc</strong>: 커맨드라인 컴파일러. `.ts` 파일을 `.js`로 변환하고 타입 체크를 수행한다.

- <strong>tsserver</strong>: 에디터용 언어 서버. 자동완성, 에러 표시, Go to Definition 등을 담당한다.
...
1. <strong>콜드 스타트 없음</strong>: JIT 워밍업 시간이 필요 없다

1. <strong>런타임 오버헤드 없음</strong>: V8 같은 중간 레이어가 없다

1. <strong>메모리 효율</strong>: 더 예측 가능하고 효율적인 메모리 관리

1. <strong>컴파일 타임 최적화</strong>: 실행 전에 이미 최적화 완료
...
아니다. <strong>컴파일러의 구현 언어</strong>와 <strong>컴파일러가 하는 일</strong>은 별개다.
...
결국 <strong>개발자 경험(DX)</strong>이 좋아지는 거지, 최종 사용자가 쓰는 앱의 런타임 성능과는 무관하다.
...
<strong>esbuild</strong>는 Go지만, <strong>SWC</strong>, <strong>Deno</strong>, <strong>Biome</strong> 등은 전부 Rust다.
